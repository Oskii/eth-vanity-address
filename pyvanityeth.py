...
def main_vanityEthAddress(prefixBytes: bytes, keyBlockCount: int, maxBlocks: int, blockIterations: int, verify: bool, verbose: bool, outputFile: str) -> int:
    ...
    if CL_PATH and CL_PATH not in os.environ['PATH']:
        new_path = os.environ['PATH'] + ';' + CL_PATH
        if len(new_path) <= 32767:
            os.environ['PATH'] = new_path
        else:
            print("Warning: Unable to add CL_PATH to PATH as it would exceed the maximum length.")
    ...
    print(f'Starting the search at {start_time}') # Print the starting time
    ...
    for n in range(maxBlocks):
        #print(f'Starting block #{n}') # Print the block number
        ...
        for i in range(keyBlockCount):
            ...
            if _ap != 0:
                print(f'Non-zero result found at iteration #{i} in block #{n}') # Print when a non-zero result is found
...
if __name__ == "__main__":
    ...
    parser.add_argument("--blockSize", required=False, type=int, default=128, help="generate block of BLOCKSIZE ethereum addresses by using GPU (default: 512)")
    ...
    parser.add_argument('--retry', action='store_true', help='keep retrying until the address is found')
    
    args = parser.parse_args()

    # Loop until the desired address is found if --retry is set
    while True:
        print("Starting from scratch...")
        print("-args.prefix: ", args.prefix)
        print("-args.blockSize: ", args.blockSize)
        print("-args.blocks: ", args.blocks)
        print("-args.blockIterations: ", args.blockIterations)
        print("-args.verify: ", args.verify)
        print("-args.verbose: ", args.verbose)
        print("-args.output: ", args.output)
        print("====================================")
        result = main_vanityEthAddress(args.prefix, args.blockSize, args.blocks, args.blockIterations, args.verify, args.verbose, args.output)
        if result == 1 or not args.retry:
            break